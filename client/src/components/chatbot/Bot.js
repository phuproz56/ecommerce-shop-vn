...
export default function App() {
  ...
  useEffect(() => {
    setMessages([
      ...
      { text: 'Vui lòng điền tên và email để tôi có thể hỗ trợ bạn!... :)', sender: 'bot' },
    ]);
    ...
    fetch("http://localhost:5000/api/chatbot/userdetails", {
      ...
      .then((response) => {
        if (response.status === 201) {
          ...
          const botMessage = {
            text: `Cảm ơn vì thông tin, ${name}. Please feel free to ask me about VIIT.`,
            sender: 'bot',
          };
          setMessages((prevMessages) => [...prevMessages, botMessage]);
          setUserData(false);
        ...
        <div className="chat-container">
          ...
          {!userData && (
            <div className="my-1 message-input">
              <input type="text" className="mx-2" id="message-input" placeholder="Nhập tin nhắn của bạn..." onKeyDown={handleKeyDown1} value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} />
              <button className="mx-1 send-button text-white" id="send-button" onClick={handleChat}> <i className="fa fa-paper-plane-o" aria-hidden="true"></i>  </button>
