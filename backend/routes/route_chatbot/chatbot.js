  ...
  try {
    const chatbotResponsePromise = new Promise((resolve, reject) => {
      const pythonProcess = spawn("python", [
        "./routes/route_chatbot/chatbot.py",
        userQuery,
      ]);
...
router.post(
  "/thumbsDownMessage",
  [body("text").isLength({ min: 1 }), body("sender").isLength({ min: 1 })],
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Thumbs down feedback saved successfully" });
    } catch (error) {
...
router.post(
  "/userdetails",
  [body("name").isLength({ min: 5 }), body("email").isEmail()],
  async (req, res) => {
