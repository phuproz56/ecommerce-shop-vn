...
const Payments = () => {
  ...
  const Rows = ({ index, style }) => {
    return (
      <div style={style} className="flex text-sm">
        ...
        <div className="w-[25%] p-2 whitespace-nowrap">
          {moment(successwithdraws[index]?.createAt).format("LL")}
        </div>
      </div>
    );
  };
  ...
  const Row = ({ index, style }) => {
    return (
      <div style={style} className="flex text-sm">
        ...
        <div className="w-[25%] p-2 whitespace-nowrap">
          {moment(pendingwithdraws[index]?.createAt).format("LL")}
        </div>
      </div>
    );
  };
  ...
  useEffect(() => {
    dispatch(get_seller_paymemt_details(userInfo._id));
  }, [userInfo._id]);
